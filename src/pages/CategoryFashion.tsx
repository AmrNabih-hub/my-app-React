...
const CategoryFashion: React.FC = () => {
  const fashion = [
    { name: 'T-Shirt', image: '/images/tshirt.jpg', price: '$29.99', brand: 'FashionCo' },
    { name: 'Jacket', image: '/images/jacket.jpg', price: '$89.99', brand: 'StyleBrand' },
    { name: 'Shoes', image: '/images/shoes.jpg', price: '$79.99', brand: 'FootwearPro' },
    { name: 'Handbag', image: '/images/handbag.jpg', price: '$59.99', brand: 'AccessoryHub' },
  ];

  const [quantities, setQuantities] = useState(Array(fashion.length).fill(1));
  ...
  const handleAddToCart = (idx: number) => {
    const { name, image, price, brand } = fashion[idx];
  ...
  return (
    <main style={{ minHeight: '100vh', padding: '6rem 1rem 2.5rem 1rem' }}>
      <section className="hero-glass" style={{ maxWidth: 1100, width: '100%', margin: '0 auto', textAlign: 'center', padding: '2.5rem 1.5rem' }}>
        <h1 className="hero-headline" style={{ fontSize: '2rem', marginBottom: '2rem' }}>Fashion & Apparel</h1>
        <div className="product-grid fade-anim">
          {fashion.map((item, idx) => (
            <div key={item.name} className="product-card">
              <span className="product-icon" aria-label="Clothing">👗</span>
